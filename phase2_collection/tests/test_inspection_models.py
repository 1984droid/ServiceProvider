...
class TestInspectionRunModel:
    ...
    def test_inspection_run_template_snapshot_must_be_dict(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            inspection = InspectionRunFactory.build(template_snapshot="not a dict")
            inspection.full_clean()
    ...
    def test_inspection_run_template_snapshot_must_have_modules(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            inspection = InspectionRunFactory.build(template_snapshot={'no_modules': 'here'})
            inspection.full_clean()
...
class TestInspectionDefectModel:
    ...
    def test_defect_identity_must_be_64_chars(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            defect = InspectionDefectFactory.build(defect_identity='short')
            defect.full_clean()
    ...
    def test_defect_details_must_be_dict(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            defect = InspectionDefectFactory.build(defect_details="not a dict")
            defect.full_clean()
